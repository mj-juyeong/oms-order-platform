...
| 작업 목록 | DB 선택 기준 정리, MySQL/PostgreSQL 비교, tenant/client 스코프 설계, 물류사 기준 마스터 설계, 핵심 테이블 설계, 인덱스 초안 작성, 마이그레이션 도구 검토, 문자열 코드 타입 정책 확정 |
...
| 작업 목록 | 공통 응답 포맷, 공통 예외 처리, requestId, 인증/인가 뼈대, JPA 설정, transaction 정책, 파일 저장 인터페이스, audit logging 인터페이스 구현 |
...
| 확인 필요 사항 | JWT/Session 선택, API Key 저장 정책, 파일 저장 위치 |
...
| 항목 | 내용 |
|---|---|
| 목표 | 상품 마스터와 배송지/차량 마스터를 업로드, 버전관리, 조회할 수 있게 한다. |
| 작업 목록 | 상품 CSV parser, Link_Area XLSX parser, tenant 기준 master version 생성, active version 관리, 중복 키 검증, 직접 매칭 검증 기준 정리, 조회 API, Frontend 업로드/조회 화면 |
| 완료 기준 | 상품 마스터와 배송지/차량 마스터를 업로드하고 활성 버전을 선택할 수 있다. |
| 산출물 | Master upload API, version API, master 조회 화면 |
| 우선순위 | P0 |
...
| 작업 목록 | 사용자/역할 관리, API Key 관리, batch audit log, API call log, download log, 권한별 메뉴, 관리자 롤백, 성능 튜닝, 대용량 페이징 |
