...
| 우선순위 | 기능 | 상태 |
|---|---|---|
| P0 | 조회페이지 스코프별 기본 흐름 정리 | 신규 추가 필요 |
...
- 조회페이지별로 필터와 배치 선택 UI는 존재하지만, 사용자 스코프에 따른 기본 진입 흐름이 아직 명확하게 고정되어 있지는 않다.
- 사용자가 물류사인지 고객사인지에 따라 고객사 선택, 배치 선택, 조회 대상 기간 기본값이 달라져야 한다.
...
- CLIENT 사용자가 로그인한 경우:
  ...
  - 배치 선택 또는 납기일/상태 필터를 통해 지금처럼 주문/Scan/PL/Label을 조회한다.
  - 기본 조회 대상은 납기일이 아직 미래인 주문 또는 오늘 이후 예정 데이터로 둔다.

- TENANT 사용자가 로그인한 경우:
  - 먼저 고객사를 선택하게 한다.
  - 고객사를 선택하지 않은 상태에서는 배치 목록/조회 결과를 바로 보여주기보다 고객사 선택 안내를 표시한다.
  - 고객사 선택 후 해당 고객사의 배치를 선택하고, 이후에는 현재 조회 화면 흐름처럼 주문/Scan/PL/Label을 조회한다.
